...
const getVip = require("../../Events/Client/vipEvent");
...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    const lastWork = await db.get(
      `${interaction.user.id}_General.WorkCooldown`
    );
    const now = Date.now();
    const vipData = await getVip(interaction.user.id);
...
Você está trabalhando como \`${job.nome}\`. Seu salário cairá jaja!`);
    ...
    setTimeout(async () => {
      await db.add(`${interaction.user.id}_Wallet.Coins`, finalSalary);

      await interaction.editReply(
        `## 🏢 EXPEDIENTE FINALIZADO\n` +
        `Você trabalhou como \`${job.nome}\`\n` +
        `💰 Salário: **${finalSalary.toLocaleString("pt-BR")} Coins**${vipText}`
      );
    }, 5000);
  },
};
